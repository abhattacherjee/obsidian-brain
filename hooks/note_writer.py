...
# Private aliases kept so every existing call site in this module (and its
# test suite, tests/test_note_writer.py, which must keep passing UNCHANGED)
# continues to work unmodified after the frontmatter-splitting logic moved to
# frontmatter.py (#277) -- see that module for the implementation and the
# docstring recording why the bound is 1000 and why the shape check exists.
_FM_MAX_LINES = MAX_FRONTMATTER_LINES
_split_lines_lf_crlf = split_lines_lf_crlf
