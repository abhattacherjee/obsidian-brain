...
- **Internal: the stdin size cap is now named for what it actually enforces (#275).** `check_items_cli.py`'s cap constant and docstrings previously said "bytes" while counting characters; both now consistently say "characters." The security test suite that guards against unbounded stdin reads now discovers every entry point automatically (via AST) instead of relying on a maintained file list, which had silently missed 5 of 8 real stdin reads.
