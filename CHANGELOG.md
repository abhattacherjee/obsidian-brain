...
### Added
- **Bash-truth differential oracle for the command gates (#334):** `TestBashTruthDifferential` builds 1053 shell constructs from a fixed prefix × suffix × wrapper product. It asks `/bin/bash` which of them actually execute the verb, using a file marker rather than stdout, and requires a deny from each of the four command gates for every one that does (663 today). Ported from the unmerged #327 review branch. A census guard fails if the executed corpus loses newlines, redirections, heredocs, single pipes, background `&`, tabs, `$'…'`, `${…}`, backticks or `$( )`. That is the shape of the corpus that once let 3035 green tests hide a 29% bypass rate. It runs in about 25 s via a thread pool. It found the PR-base gap above on its first run. Against the #327 branch's intermediate push gate (31438f8) it reports 339 of 663 constructs allowed.
- **`/vault-doctor --check crossed-source-session` — detect mis-attributed notes (#330):** a note whose `source_session` contradicts the `session_id` of the note its own `source_session_note` wikilink resolves to has been attributed to the wrong session, and nothing surfaced that before. Detection only — `apply()` always returns `skipped` and never writes, because deciding which of the two ids is correct needs evidence the check does not have, and a wrong auto-repair would rewrite genuine attribution; the skipped result says so, so a `--apply` run explains itself. Confidence 0.95, so `--min-confidence` up to 0.9 does not silently drop it. It reports crossed pairs **only**, never dangling links: a dangling backlink is the normal mid-session state, because PreCompact writes a snapshot's forward reference before SessionEnd creates the parent note. That was confirmed live while this shipped — a peer session's perfectly healthy retro (its `source_session` hashes to `83f5` and its backlink names `83f5`) pointed at a note that did not exist yet because the session was still running. The live vault holds 40 such links against one genuine crossing, so reporting them would bury the signal; `snapshot-integrity` and #214 own that space. **Fixed before release (adversarial review):** the check read every note through `obsidian_utils.read_note_metadata()`, whose session cache does a `cache_get`/`cache_set` round-trip against `~/.claude/obsidian-brain/cache-<sid>.json` on every call — with the check's own unbounded scan (`DEFAULT_WINDOW_DAYS = 9999`), that balloons the cache file (measured 1.32 MB on a 2472-note vault) and slows every later cache read in the session, and its no-invalidation cache key (`metadata:<realpath>`, no mtime/size/hash) meant a re-scan after a human hand-fixed a crossed note's `source_session` still reported it unresolved. It now reads notes through a pure, uncached parser (the same one every sibling check uses to avoid the session cache), closing both. Also fixed: the two unguarded `folder_path.is_dir()` calls could raise `PermissionError` on Python 3.13 (a parent directory at mode `0o000`) instead of returning `False`, and one sits inside the loop over both scanned folders — an unguarded raise on the first would abort the scan and report clean having examined nothing; both are now wrapped.
- **`/vault-doctor --check memory-index` — detect memory-index drift (#308):** writing a Claude Code memory entry is two steps (create `memory/<name>.md`, then add a pointer line to that store's `MEMORY.md`) and only the first is enforced by anything, so a skipped pointer makes the entry silently unrecallable forever. The new opt-in check walks `~/.claude/projects/*/memory/` and reports orphaned entries (transitive BFS from `MEMORY.md` — `orphan-isolated` when nothing links the entry at all, `orphan-unreachable` when only another orphan does), dangling index pointers, stores holding entries but no `MEMORY.md`, unreadable entries, and `MEMORY.md` size against the ~17 KB compaction threshold and ~24 KB read budget. Link matching is deliberately asymmetric: reachability counts markdown links, `[[wikilinks]]` and bare `name.md` mentions, because under-counting invents orphans that aren't there; dangling counts markdown links **only**, and only ones naming a direct child of the store, because `MEMORY.md` legitimately wikilinks Obsidian vault notes, prose cites `CLAUDE.md`, and a `reference` memory line may link a URL ending in `.md`. A pointer whose target differs only in case is `index-case-mismatch`, not dangling — it resolves on a case-insensitive filesystem and breaks on a case-sensitive one. An index with text in it that names no file at all, over a store that holds entries, gets an `index-names-nothing` summary row alongside the per-entry orphan rows: the index read is strict, so anything reaching that state decoded cleanly and genuinely names no file, which makes every orphan row correct. A `--project` value matching no store reports a `project-filter-matched-nothing` row instead of exit 0 and an empty report — a typo in the filter used to be indistinguishable from a clean machine. Dangling detection now covers every CommonMark form that names a direct child (`](name.md)`, `](./name.md)`, `](name.md "Title")`, `](<name.md>)`); the three latter forms were silently missed, and a missed dangling pointer reads as clean. Reachability folds case across all three link kinds rather than markdown links only, because `[[Their-Name]]` is the documented way one memory links another and that is where a case-only difference lives. An unreadable entry gets one `entry-unreadable` row and no orphan row on top; it stays in the entry set, so the index pointer at it is not misreported as dangling. `vault_doctor.py`'s `--json` writer now carries this check's counts, sizes, index path, byte offset, both spellings of a case mismatch, the store counts behind a filter miss and the underlying read error, so `skills/vault-doctor/SKILL.md` no longer has to re-parse them out of `reason`; the keys are conditional, so no other check's row shape changed. `MEMORY.md` is decoded strictly: replacing bad bytes there would report the whole store as orphaned, so an index that is not valid UTF-8 makes that store unreadable. Entries are decoded leniently but get an `entry-undecodable` row, and any entry that could not be read in full downgrades that store's `orphan-isolated` rows to `orphan-unreachable`, because its links are unknown. An unreadable store is dropped from the report and gets one `store-unreadable` row naming it; `scan()` raises — exit 2 with `crashed_checks` — only when **every** selected store fails, so one bad store can no longer empty the whole report. The check is report-only — every row is unresolved at confidence 0.0, so `fix` never writes to a memory store (and `--min-confidence` above 0.0 hides every row). `--days` is ignored (the drift is undated) and `--project` is a case-insensitive substring match on the project directory name. Opt-in because a full run on the author's machine produced 92 rows across 7 projects (measured 2026-08-12), which would drown the default sweep — this repo's own store measured 221 entries, 63 of them unreachable.
- **`/retro`'s evidence bundle now spans a compact boundary and surfaces prior retrospectives (#184, #285):** `gather_session_evidence()` gains a keyword-only `also_session_ids`, so a session that resumed from a continuation summary can pass the prior session id (recovered from the transcript path the continuation preamble cites) alongside the current one, and evidence written under either id — snapshots, insights, decisions, error-fixes — is folded into one bundle. Snapshots are deduplicated by stem across ids; every entry in the bundle now carries the `session_id` it actually came from. `/retro`'s `(pre-compact arc)` marker uses this today for snapshots only — insights, decisions, error-fixes and retros carry the field but are not yet labelled by it. The normalized, de-duplicated list of ids actually scanned is exposed as `bundle["session_ids"]`; ids equal to `""` or `"unknown"` are dropped, and a primary id of `"unknown"` no longer suppresses discovery when a valid id was recovered via `also_session_ids`. Separately, the bundle now collects prior `claude-retro` notes into their own `bundle["retros"]` — previously such a note fell through the type-bucket filter and vanished silently. `/retro` Step 3a scopes each new retrospective to the arc after the most recent prior retro; retros are a boundary marker, deliberately excluded from the per-snapshot "every RELEVANT snapshot must surface in the analysis" coverage rule.
- **`/check-items` gains a non-git evidence source for notes-only projects, hardened through five follow-up rounds (#318):** every evidence source `deep_analysis_pipeline` gathered was git-derived, so a project with no local repo could never reach `DONE` even when the vault already said so. `gather_note_completion_evidence()` mirrors `/recall`'s `contradicted_by` signal — an open item is flagged when a strictly newer session's own `## Summary` reports it done, gated behind the same completion-phrase guard the heuristic classifier uses so a mere co-mention can never fabricate DONE evidence. Wired end to end: the pipeline attaches `note_completions` per project regardless of `repo_path`; a new prefilter Rule 0 classifies a group on an exact normalized-text match only, never a blanket pass; and the sub-agent can cite it as `"reported done in session YYYY-MM-DD (<title>)"`, capped at tier MED like every non-high-trust source. Making that cap actually hold took five follow-up rounds: `assign_tier` initially tested only the citation's *wording*, and five off-template phrasings (one differing by a single word) all reached HIGH and auto-checkoff anyway — text-shape matching cannot enforce a safety property against a model's own prose. The guarantee is now `note_evidence_only`, computed once per project from the evidence bundle's own keys (a data property the pipeline controls, not model prose) and threaded through every producer of a classification record, including the cache-replay path, which was still stamping the unsafe default on every re-run after the one that first populated the cache. The underlying predicate was also key-presence, not value-aware: `deep_analysis_pipeline`'s git block sets every git-derived key to `[]` on failure (no tags, an unauthenticated `gh`) — the normal state on a fresh machine — which the original check misread as "has git evidence" and left a repo-backed project with zero real evidence uncapped, the mirror-image bug to the one this cap exists to prevent.
- **`/check-items` now warns when installed `obsidian-brain` copies disagree on version (#318):** Claude Code loads a skill's `SKILL.md` from whichever install it registered while the hooks it calls resolve independently (preferring the marketplace directory-source install), so the two can legitimately diverge with nothing checking — reproduced on the development machine: `SKILL.md` loaded from a `3.4.1` cache while hooks resolved from `3.5.0`, every step ran, nothing warned. A skill cannot introspect which copy of its own `SKILL.md` was loaded, so rather than comparing "this skill" against "these hooks", `describe_plugin_install_divergence()` (`hooks/obsidian_utils.py`) enumerates every `obsidian-brain` install on disk — the directory-source checkout plus every `~/.claude/plugins/cache/*/obsidian-brain/*/` — and warns, to stderr and never failing the run, when their versions disagree, naming which one the hooks actually resolved to. Step 1 calls it and swallows any failure; the fix it points to is `/plugin marketplace update`, or removing the stale cache directory.

### Changed
- **`/check-items` dashboard's `applied` count now means "records stamped applied", not "occurrences flipped" (#318):** the report always accepted `applied` as a caller-supplied run total. It's now derived by counting classification records whose `applied` field is truthy — one per **group**, not one per checkbox occurrence in the vault. A group with three sibling lines cascaded together now reports `applied: 1` (one group applied, with 2 more flips folded into `cascaded`), where a caller previously might have reported 3. The frontmatter `applied:` field and the `## Summary` line reflect this on every dashboard note going forward; historical notes are unaffected.

### Fixed

- Claude lifecycle hooks now check the transcript path before inherited Codex environment markers. All four hooks skip Codex rollout payloads with `SKIPPED_CODEX_HOST` while the Codex host adapters are pending. SessionEnd logs this outcome in place of `SKIPPED_TRANSCRIPT_OUTSIDE_PROJECTS` for Codex sessions.
- **The PR-base gate judged `gh pr create` and `gh pr merge` only at the start of a command or after `;`/`&`/`|` (#334):** after a newline, a tab, `VAR=x `, `$( )`, a backtick, `( )` or `{ }`, both verbs ran unchecked. That let a feature PR target `main`, or a `feature` → `main` merge skip its base check. Both now match anywhere, like the commit gate. Each occurrence is now judged on its own arguments, which are read quote-aware (`shlex`) up to the next command separator. Every base a create passes, in any spelling `gh` accepts (`--base X`, `--base=X`, `-B X`, `-BX`, `-B=X`, `-dB X`), must be `develop`. The gate does not guess which one `gh` keeps. A merge is checked against its selector (a number, `#N`, a pull URL or a branch) and against every other token that looks like a PR reference. So a flag-parsing difference such as `-st 5 7` can only add a check, never move it to the wrong PR. A selector computed by a command substitution, or arguments with unbalanced quotes, are denied. The gate no longer stops after the create check, so a merge in the same command is always judged. A command that only mentions a verb (an `echo`, a `grep`, a commit message) is now judged too and usually denied. That is the fail-closed direction the other gates already take.
- **Codex-run skills no longer stamp a Claude session's id on vault notes (#362):** when a Codex marker (`CODEX_THREAD_ID`, `CODEX_SANDBOX`, `CODEX_SANDBOX_NETWORK_DISABLED`) is set, `_resolve_session_id` returns `unknown` before any layer runs. Before this, a Codex process took either the `CLAUDE_CODE_SESSION_ID` it inherited from a Claude Code shell or the newest Claude transcript in the repo, both of which belong to the Claude session open alongside it. Skills already handle `unknown` by leaving out `source_session_note`. `CODEX_HOME` is not a marker, because it is user config. Since every Codex run now resolves `unknown`, the session cache refuses that id outright (`cache_get`/`cache_set`/`cache_invalidate`). Before, a shared `cache-unknown.json` that nothing ever cleaned up served frozen config and note frontmatter to every such process. `check_hook_status` reports Claude logging as "not checked" under Codex rather than telling the user to re-run `/obsidian-setup`. The one affected note, `2026-09-24-retro-2be8.md`, was restamped to `source_session: unknown`.
- **`/check-items` no longer checks off DONE items you deselected (#340):** Step 8 cascaded every DONE group, including a DONE you deselected and a MED DONE you never opted into. The vault ended up checked while the Step 9 report, which renders from the `applied` stamp, showed the item open. The cascade now runs only for groups whose own line you actually flipped. Every cascaded group is therefore one the report shows as checked. Two cases that used to cascade no longer do: an item whose primary flip was aborted on a text mismatch, and an item whose line you had already checked by hand. Since the stamp now decides whether the cascade runs, Step 8 compares real paths, so a symlinked vault still matches. It also warns when recorded flips match no group, or when the flips file fails to load, instead of printing a quiet `cascaded_total=0`.
- **An unreadable sessions folder no longer silently reads as "no session note" (#336):** `Path.glob()` swallows the directory's `OSError` and returns nothing. So `_resolve_session_note_by_hash` reported no match, printed no diagnostic, and let the caller compose a fresh name that could duplicate the session note. It now lists with `iterdir()`, which raises, so the existing diagnostic and fallback run. The retro-gate reaper gets the same swap. The `rglob` transcript fallback keeps its handler, with a comment saying what it does and does not cover. The old test patched `Path.glob` to raise, which the real `glob` never does; it now uses a real `0o000` directory.
- **`get_session_context()` no longer hands concurrent sessions each other's id (#330):** session resolution walked `~/.claude/projects/<project>/*.jsonl` and took the newest mtime. Two live sessions in one repo both append constantly, so that pick is a coin flip and each session could get the other's id — observed live, with two retros written seconds apart each stamping the other's `source_session`. It was also *unstable* rather than merely wrong: a real vault note carries `source_session` hashing to `f157` while its backlink names `df46`, because two calls inside a single note write resolved via different winners, and the existing cache cannot pin that since the cache key *is* the resolved sid. Resolution now starts from the harness's own `CLAUDE_CODE_SESSION_ID`, validated by format only and trusted ahead of every scan layer — it is the id Claude Code assigned to this process, so it is authoritative regardless of which transcript was touched last, and constant for the session, so repeated calls agree. It is deliberately not gated on the transcript existing, since a brand-new session has no JSONL yet and falling through in that window is the bug itself. Side effect: the per-note hot path got faster, because the env layer answers from an environment variable rather than globbing the filesystem on every read. Measured on the development machine (one machine, order-of-magnitude only, not a portable constant): the env layer read in ~9.7µs against ~15.9ms for a glob over the 2259 transcripts in *this project's* `~/.claude/projects/` directory — a few orders of magnitude apart. Where the env var is unavailable, resolution falls back to newest-mtime over the project directory, same as before this change — a stricter concurrency-ambiguity refusal was tried and then reversed after adversarial review; see `docs/plans/330-session-id-crossing.md`. **Fixed before release (adversarial review):** layer 0's env-sid existence probe globbed a NARROWED suffix (one specific sid's filename) through the same restriction path (`_restrict_matches_to_cwd_project`) the broad, default-suffix glob uses, and that path's `_dirs_by_transcript_cwd` memo is keyed only by `(cwd, sorted(dirs))` — not by which files were inspected — so the narrow probe's "cannot tell" verdict (a brand-new transcript's first line usually has no `cwd` field) got memoized under the same key the later broad glob would hit, silently downgrading a genuine #260 "positively contradicted → refuse" into "cannot tell → keep anyway" for files the narrow probe never looked at. The existence probe now globs directly, bypassing that shared memo entirely — it only needs to know a file exists, not which directory owns it. Also fixed: `_warn_once()`'s check-then-add was not atomic, so concurrent `upgrade_batch()` worker threads could both pass the membership check before either registered the key and print the same one-shot warning more than once; it now holds a lock across both steps.
- **The retro classification gate cannot be armed under a dead key (#330):** the gate is armed by the retro skill and checked by the Stop hook from the harness's stdin `session_id`. When those diverged, each session armed a gate the other's hook could not see and enforcement silently vanished. The env layer makes both sides the same value by construction, and `mark_retro_classification_pending()` now refuses outright on an empty, whitespace, or `unknown` id, returning a visible `Failed:` string the skill already echoes — arming under a key the Stop hook will never look up is strictly worse than not arming. Adds the cross-source test that was missing: arm through `get_session_context()`, then drive the real Stop hook as a subprocess with the harness id.
- **`source_session_note` is no longer written when the target disagrees (#330):** a new `resolve_source_session_note()` helper returns the wikilink stem only when the target note exists *and* its own `session_id` matches the stamped `source_session`, and `decide`, `error-log`, `compress` and `retro` now call it instead of composing the field blind. Note this is advisory rather than enforced: `note_writer.py` is content-blind and never composes these fields, so the frontmatter is assembled from SKILL.md prose. The helper is also path-contained (`resolve()` / `is_relative_to()`, matching `write_vault_note()`), since `session_note_name` is composed from a slugified cwd basename rather than being a literal. Snapshots are deliberately exempt — their `source_session_note` is a forward reference to a parent note SessionEnd has not created yet, and `vault_index.py` parses it to pair snapshots with parents; a regression test pins that behaviour.
- **A global option before the verb skipped every `.claude/hooks/` gate; `--mirror`/`--prune` and glob refspecs could rewrite `main`/`develop` with no `--delete`; and the tag allowlist ran three unanchored substring tests ahead of the deletion gate (#351):** `git -C . push origin main`, `git -c k=v push origin main`, `git --no-pager push origin main`, and the `gh --repo o/r pr create`/`gh -R o/r pr merge` equivalents all measured ALLOW on `develop` at `74cf1b1`, across all five gates — each verb regex matched only `git push`/`gh pr create`/`gh pr merge` immediately after the executable, so a global option ahead of the verb was never seen and every check below it was skipped. A shared `_GLOBAL_OPTS` regex fragment (zero-or-more `-`-prefixed options, each optionally taking a separate quoted or bare value) is now spliced into the verb pattern of all five hooks (`_PUSH_VERB`, `_CHECKOUT_VERB`, `_COMMIT_VERB`, `_PR_CREATE_VERB`, `_PR_MERGE_VERB`); `gh -R`/`--repo` is deliberately **not** descoped the way `git -C` is, so a `-R`-redirected `gh` command is still gated by this repo's rules. Separately, `prevent-direct-push` now denies `--mirror`/`--prune` outright — the hook cannot know which remote refs they would delete without querying it — prefix-matched against git's own abbreviation rules (`--mi`/`--mir` mirror `--mirror` identically) and extended to `--all`/`--branches` (`_all_ref_flags`) and to any refspec whose destination glob could match a protected ref (`_branch_glob_refspecs`, via `fnmatch` against `main`/`develop` and their `heads/`/`refs/heads/` spellings). The tag allowlist's three substring tests are replaced by one ref-token predicate (`_is_tag_ref`/`_VERSION_TAG`, `fullmatch` on a single token) that sits below the deletion gates, so `git push --tags origin --delete main` — which used to delete `main` — now denies. Three more live fail-opens on `develop` were found and fixed in the same pass: a protected destination is now decided as a **ref token** anywhere in the argument list (`_protected_push_refs`/`_protected_delete_refs`), not only adjacent to the remote, closing `git push origin foo main` (the old `targets_protected` tested the literal substring `"origin main"`) without denying an ordinary branch named `maintenance`; a quoted flag (`"--delete"`) escaped every flag test through the whitespace-split fallback taken on an unbalanced quote, fixed alongside the same gap in `_all_ref_flags`; and a quoted `;` inside a global-option value, a backslash-escaped separator, and a ref arriving from `$( )` with a trailing `)`/`` ` ``/`}` unstripped each cut the invocation apart before the deletion gate could see the refs, fixed in `_push_invocations`/`_bare_ref`. `_bare_ref` also normalises shell punctuation glued to the ref token, which bash splits off but this hook does not: `git push origin main>/dev/null` really pushes the branch, and bash strips the trailing backslash from `git push origin main\`. Measured across 84 generated rows — two protected refs x six push forms x seven punctuation forms (`>f`, `>>f`, `>|f`, `<&-`, `<>f`, `>/dev/null`, a trailing backslash), every one valid bash — all 84 denied at `74cf1b1` via the old substring arm, and the ref-token rewrite had to normalise the glue to keep them denying, including the deletions among them. A redirection is cut at the first `<`/`>` rather than stripped from the right, since the punctuation is not always last and `git push origin >main` is a redirect into a file of that name rather than a push of it. `prevent-direct-push` splits a compound command into one invocation per push before any of those checks run, and its quoted-span scanner (`_quoted_spans`) is what decides whether a given `;`/`&`/`|` is a real separator or sits inside a closed quoted run — it belongs to that split, not to the shared verb patterns above. Testing each separator against every span was quadratic; a `bisect` over the sorted span offsets makes it linear, and a 128 KB adversarial shape carrying many of both fell from ~37.9s to ~67ms. Known gaps, all pre-existing on `develop` and not regressions: three quoted spellings of a global-option token (a leading dash inside quotes, or every character backslash-escaped) remain fail-open, pinned as strict `xfail` rows in `tests/test_security.py` so a future fix can't pass silently without deleting the ledger entry — and those pinned rows are a sample of a wider class rather than the whole of it: the **executable and verb words themselves** can be quoted or escaped (`git "push"`, `git p'u'sh`, `git pu\sh`, `"git" push`) and are not normalised, so a mangled verb skips the entry test on all five gates; this is unchanged from `develop` and is a wider surface than the three pinned rows suggest; a `#` comment, an unbalanced quote inside one, a `case … )` pattern, or a heredoc body ahead of a `git -C` are blind spots in the shared `_shell_scan` scoping helper and now cost a fail-**closed** false deny on legitimate cross-repo `-C` work, the accepted cost of treating an unvouched `-C` as not provably real; and a mirror configured via `remote.<name>.mirror` rather than the `--mirror` flag is invisible to command text and is not detected.
- **Deleting a protected branch alongside a release ref is now blocked (#333):** `prevent-direct-push` stood the whole hook down for branch cleanup when `--delete` appeared **and** `release/` or `hotfix/` appeared anywhere in the command. Neither half was tied to an argument, so a protected ref riding alongside a release ref was permitted: `--delete origin release/x main` deleted `main`, while the plain `--delete origin main` denied. A deletion is worse than a push to the same branch — a bad push can be reverted, a deleted ref is gone. Tightening the allowlist alone would not have fixed it, so there is now a deny of its own: nothing downstream caught that command either, because `origin main` is not a substring of it, the refspec arms need a `:` or a `heads/` spelling, and on a feature branch the current-branch test is clean. Both halves are decided on **ref tokens** now — the arguments of each `git push` in the command, quotes stripped, first non-flag token read as the remote. A protected ref among the refs being deleted denies, in every spelling git accepts (`main`, `heads/main`, `refs/heads/main`, `+`- and `:`-prefixed); the stand-down needs **every** ref to be a `release/` or `hotfix/` one, not merely one of them. `-d` counts as `--delete`, including bundled as `-fd`, which is how git's own parse-options reads it. Ordinary branches whose names merely begin with a protected name (`mainline`, `maintenance`) are untouched, and deleting a `feature/` branch — what `git-branch-cleanup` actually runs — behaves exactly as before. The deny message names deletion rather than the Git Flow push workflow, which is the wrong remedy for a lost ref. Three defects found while reviewing the fix are folded in: a backslash line-continuation was not collapsed before the command was split, so a protected ref written after one landed in a segment with no push verb and was dropped (the same collapse `_targets_this_project` already did); the whitespace fallback taken when `shlex` cannot parse a segment left the quote character attached to the token, so an unbalanced quote in front of a protected ref hid it; and `shlex` is quadratic in the length of a single argument, which took a ~900 KB command from 0.03s to 5.6s in a hook that runs before every Bash call, so it is now skipped when there is no quoting to strip or the argument string is over 100 KB. The first two were measured as ALLOW and the third as a ~185x slowdown; all three are covered by tests, and every guard in the gate is mutation-proven. A fourth, a false DENY, is fixed too: a flag whose value is a separate token (`-o`, `--push-option`, `--repo`, `--exec`, `--receive-pack`) shifted remote detection by one, so `-o ci.skip --delete origin release/x` read `ci.skip` as the remote and `origin` as a ref, no ref was a release ref, the stand-down was withheld, and a legitimate release cleanup run from `develop` was refused. Those five flags now consume their value token. Only an unprefixed token is consumed, so an over-broad entry cannot swallow a real option, and the `=`-joined spellings need no entry because they are already a single `-`-prefixed token. Checked against real git rather than assumed: `--delete -o origin main` and `--repo origin --delete main` both make git read the remaining positional as the REPOSITORY and refuse with "--delete doesn't make sense without any refs", leaving the remote untouched, which is exactly what the new skipping predicts.
- **A refspec push to `main` or `develop` is now blocked (#327):** `prevent-direct-push` built its `targets_protected` test to include refspec spellings, under a comment saying it detected them — and then the very next `if` re-tested a strictly narrower condition (the current branch, or the literal `origin main`/`origin develop`) that no refspec form can satisfy. Every refspec arm was computed and discarded, so `git push origin HEAD:main`, `mybranch:main`, `+main:main`, and both `--force` and `--force-with-lease` spellings all fell through to allow. A force-push over `main` was permitted in practice. The narrowing `if` is removed, so `targets_protected` decides. The refspec arms match the protected name as a **whole ref** in the same change, because a plain `":main" in command` substring test also fires on `foo:mainline` and `HEAD:maintenance` — measured, both denied once the condition started deciding anything, and both are ordinary branches. `origin main` still matches `origin maintenance` as a substring; that false deny predates this change and is left alone here. The refspec test covers the qualified spellings too — `HEAD:heads/main`, `HEAD:refs/heads/main`, and a qualified ref standing alone with no colon (`refs/heads/main`, `+refs/heads/main`), where git pushes the branch to itself. The `--delete` allowlist named in the original note as a known gap is closed by #333, below.
- **A trailing `cd`, or a NUL byte, no longer switches off every `.claude/hooks` gate in this repo (#326):** `_targets_this_project()` — hand-duplicated into all five PreToolUse gates so `/harden-repo` can install each file standalone — exists to stand a gate down when a command targets a *different* checkout. It found the `cd` with a bare `re.search`, i.e. the first one **anywhere in the command, including after the dangerous verb**, where it has no bearing on what the verb did. Appending `; cd /nonexistent` to any command therefore disarmed the lot: returning `False` here exits 0 with nothing on the decision channel, and in the PreToolUse contract that is an ALLOW. Measured against the unfixed code with `CLAUDE_PROJECT_DIR` set (its value in real operation), all five gates went from deny to allow — preflight enforcement, branch-name validation, PR-base enforcement, protected-branch push prevention and the changelog gate, all defeated by eleven trailing characters. The helper now takes the verb the caller already matched and, for each occurrence of it, honours only the **last `cd` that precedes it** — last-write-wins, as in the shell — returning "in scope" unless *every* occurrence is provably elsewhere. A `cd` counts only when `&&` is the sole thing between it and the verb: `&&` runs the verb only if the `cd` succeeded, whereas `cd /elsewhere ; <verb>` runs the verb *here* the moment the `cd` fails, and whether it fails is not knowable from the command text. **The second route in was a crash.** `os.path.realpath` raises `ValueError` on an embedded NUL and nothing caught it, so `cd /tmp/<NUL>x && <verb>` was a traceback and rc 1 on 5/5 hooks — and rc 1 is a *non-blocking* error, so the command proceeded. Path resolution is now wrapped, on the `cd` target and on `CLAUDE_PROJECT_DIR` alike, and an unresolvable path reads as **in** scope. Every other way scope can fail to resolve — no `CLAUDE_PROJECT_DIR`, an unusable verb pattern, a verb the caller matched but the helper cannot locate — now returns "in scope" for the same reason. **`enforce-pr-base-branch` ran its scope check on every Bash command**, at module level ahead of any verb guard, which is what gave the NUL crash its widest exposure; each verb branch now runs its own check, so it only executes for a command that gate was going to judge anyway. The five copies are made byte-identical and a drift test pins them that way, since duplication is deliberate here and drift is how one gate silently keeps a bug the others were fixed for. **One behaviour change loosens rather than tightens, and is worth naming:** the prefix comparison was `target.startswith(project_dir)`, with no separator, so a sibling directory that merely shares a path prefix — `/path/to/obsidian-brain-evil` against `/path/to/obsidian-brain` — read as *inside* this project and the gate fired on it. The compare is now anchored (`target == project_dir or target.startswith(project_dir + os.sep)`), so those commands **allow** where they previously denied. That is correct scoping — a sibling directory is a different repo — but if you were relying on a gate firing for a neighbouring checkout, this is the change that stopped it. **A `cd` the shell never executes no longer disarms anything either.** The `cd` regex matches raw command text, so it honoured a `cd` inside `"..."`, `'...'`, `` `...` `` or `$(...)` — none of which move the shell — and `echo "x; cd /tmp" && <verb>` therefore ran the verb HERE while the guard concluded it ran elsewhere. Measured before the fix: four such shapes, all five gates, every cell an allow. This route is older than the trailing-`cd` one (it defeats the first-`cd`-anywhere search just as well), and it falsifies the invariant the guard now claims for itself — a `cd` that never ran proves nothing about where the verb ran. A `cd` match is now admitted only when a scan of the text before it (`_shell_scan`) ends outside every quote, backtick and `$( )`; anything that scan cannot parse drops the `cd`, which leaves the command in scope. **A `cd` inside `( )` now descopes the rest of that subshell, and only that.** `(cd /other/repo && <verb>)` is the form CLAUDE.md mandates over a bare `cd`, and it used to be gated — the `cd` regex never matched after a `(`. It matches now, but the subshell has to still be OPEN at the verb: `(cd /other) && <verb>` and `(cd /other && true) && <verb>` still gate, because the verb runs after the `)` puts the cwd back. A `cd` inside a substitution never descopes at all, even when the verb is inside the same one — deliberately over-strict, since a substitution's interior is where this scanner is least sure of itself. **Two connector changes affect commands that merely wrap lines.** `\` + newline is a line continuation — whitespace — but the newline reached the `&&`-only rule and re-scoped a command that genuinely ran elsewhere, so `cd /other/repo && \` + newline + `<verb>` denied where it used to allow; the continuation is now collapsed before any position is computed. A **bare** newline between a `cd` and the verb keeps the new stricter reading and denies: it is a separator, so a failed `cd` runs the verb here, which is the same reason `;` does not descope.
- **The stdin-cap security guard now watches the tree it was missing, and five uncapped hook entry points are capped (#289):** `tests/test_security.py::TestStdinCap` parses the AST of every project module and fails any stdin read it cannot prove is bounded — but it only ever walked `hooks/` and `scripts/`. The five PreToolUse hooks under `.claude/hooks/` (`require-preflight`, `enforce-pr-base-branch`, `prevent-direct-push`, `update-changelog-before-pr`, `validate-branch-name`) are stdin entry points by definition and are covered by the same CLAUDE.md rule, and every one of them read `json.load(sys.stdin)` unbounded while the guard stayed green. All five now read through the project's capped idiom (`json.loads(sys.stdin.read(1_000_000))`); what each hook does once a payload it cannot use reaches that cap is the subject of the fail-open fix described below. In the same pass the opt-in `OB_SCAN_RESOLVED_INSTALL=1` walk over the resolved plugin-cache tree (added in #278) is **removed** rather than repaired, because it could not catch anything: it short-circuits on a directory-source install (the resolver points back at the checkout) and finds nothing on CI (no registry, no cache), so it was a no-op in both configurations that occur; and a plugin cache is populated from a release cut from this repo, which puts the repo walk *upstream* of every byte that can land in one — the only uncapped cache trees possible predate #275 and are unfixable from any checkout by construction (measured: the 3.4.1, 3.4.2 and 3.5.1 caches on the development machine all scan clean, and the 3.3.0/3.2.2 caches the issue named are gone). Its hand-written mirror of the #278 resolver went with it; that resolver's behaviour is covered against the real SKILL.md bytes in `tests/test_hooks_resolver_drift.py`, so no coverage was lost. `test_discovery_finds_the_known_entry_points` now names all five `.claude/hooks/` files — the load-bearing half, since those files are capped now and the cap assertion would be green over them whether or not the walk reached them. **Capping the read surfaced a second, larger defect on the same lines: three of the five hooks failed OPEN on input they could not read.** `enforce-pr-base-branch`, `prevent-direct-push` and `validate-branch-name` all did `sys.exit(1)` on a JSON parse error — which reads as refusal but is not: a non-zero exit is a *non-blocking* error in Claude Code's PreToolUse contract, so the error is surfaced and the tool call proceeds. A malformed payload therefore walked straight past the protected-branch push gate, the branch-name gate and the PR-base gate. (`require-preflight` already got this right, with the comment "Security gate must fail closed, not open"; that is the shape the other three now follow.) All five hooks now read through one `_read_hook_input()` helper that reads `_STDIN_CAP + 1`, treats overflow as its own condition, and routes both overflow and a parse failure to the hook's existing deny/block path — `exit 0` carrying `permissionDecision: "deny"`, which is what actually blocks. The explicit overflow branch is not cosmetic: an oversize payload whose first `CAP + 1` characters are *still valid JSON* (a complete document followed by whitespace) would otherwise parse cleanly and the hook would decide on a truncated view of the payload. Capping and catching a parse error is not the whole of it, though: three more shapes were measured exiting non-zero — i.e. fail-open — on all five hooks, and the helper now denies each. **The read itself** was outside the `try`, so invalid UTF-8 on stdin raised `UnicodeDecodeError` before `json.loads` was ever reached; it is now inside a handler as broad as the parse's, because a narrow one still missed a closed fd 0 — CPython sets `sys.stdin` to `None` there, so the failure is an `AttributeError` on the attribute access and `read()` is never entered. (The guarantee's floor is the interpreter: a *directory* on stdin aborts CPython in `init_sys_streams` before the hook's first line, which no in-script handler can reach.) **The parse** caught only `(JSONDecodeError, ValueError)`, which ~400 KB of nested `[` — comfortably under the cap — walks straight past with a `RecursionError`; it is now a bare `except Exception`. **The shape** was unchecked: `null`, `[]`, `"5"`, `5` and `true` parse cleanly and then blow up on `.get()` in the caller, and so do `"tool_input": "…"` (`'str' object has no attribute 'get'`) and `"command": 5` (`argument of type 'int' is not iterable`) one line later — the payload must now match what the callers assume, dict / str / dict / str. `TestHookInputFailsClosed` pins all of it behaviourally against the real scripts: nine unusable payloads deny, a payload of exactly 1 MB is allowed while 1 MB + 1 denies (the boundary operator was previously pinned only by a `CAP + 100` fixture, so `>` -> `>=` left the suite green), and a benign **Bash** command is not denied — Bash because all five hooks are registered with `"matcher": "Bash"`, so a non-Bash control exercises a path production never takes. Two coverage gaps the refactor exposed are closed alongside it: `TestStdinCap` regains a negative control for its own assertion (`#278` got that property incidentally from the resolved-tree test this change removed; without it, mutating the bound resolver to report every read as capped left the whole module green), and its entry-point count is now exact rather than `>= 13`, with all 13 named. Finally, the five hooks' **business** deny paths — protected-branch push, non-Git-Flow branch name, `gh pr create` with no `--base`, commit without a preflight token, PR with an empty `[Unreleased]` — get their first automated coverage in `TestHookBlockingPathsFire`, against a throwaway git repo rather than the checkout, since an in-tree probe of the changelog gate passes or fails purely on what the developer's branch happens to carry. **One behaviour change is worth naming explicitly:** `_read_hook_input()` is now the first statement in every hook, so it runs *before* `_targets_this_project()`, and an unreadable payload therefore denies even when the command targets a repo outside this project — where it previously proceeded. That is deliberate rather than incidental: project scope is derived from `command`, which is exactly what an unreadable payload does not supply, so "not my project" is not a conclusion the gate is entitled to draw. If you hit one of these denials while working in a different repo, this is the change that caused it. **A deny that cannot be written now exits 2 instead of exiting 0 with nothing on the decision channel.** `exit 0` blocks only because of the JSON on stdout, so a deny the hook cannot write was an *allow* — silently, with no traceback and no stderr (measured with fd 1 closed: all five hooks gave rc=0 and zero bytes on both streams). `_deny()`/`block()` now flush explicitly inside the try — without that, a broken pipe surfaces only at interpreter shutdown, after the exit status is already fixed — drop `sys.stdout` before exiting so CPython's finalization re-flush cannot override the status with 120, and report the reason via a truthiness-guarded `sys.stderr.write` rather than `print(..., file=sys.stderr)`, which falls back to *stdout* when fd 2 is closed. `sys.stderr` is dropped alongside `sys.stdout` (after the reason is written): when both streams are the same dead pipe, dropping stdout alone still exits 120.
- **`/check-items`'s #306 cache lock closes nine ways it could still lose data or go quiet without saying so (#323):** a follow-up silent-failure review, every finding verified by execution. `load_cache()` collapsed three different causes of an empty result into one: a genuinely corrupt file (where overwriting on save IS the correct recovery), a transient `OSError` reading an otherwise-fine file (EACCES, EIO, EMFILE), and a schema mismatch (valid JSON from a different/newer `schema_version`) — the latter two were never proven bad, yet `locked_cache()` saved over them unconditionally, silently replacing every other project's verdicts with this run's alone. `load_cache()` now takes an additive, opt-in `with_status=True` that returns `(data, status)` — existing bare callers (`skills/check-items/SKILL.md` Step 3, `scripts/dev-test/test-issue-87-manual.py`) are unaffected — and `locked_cache()` refuses to save on `"unreadable"`/`"foreign_schema"`, with a warning that says plainly the update is being dropped and the existing cache is untouched; a legitimately empty-but-present cache (reachable after GC) still carries `status=None` and saves normally, so the cache can always repopulate (F1). Lock-ownership loss was detected only in `_release_lock`, *after* `save_cache()` had already run — too late for the warning to describe an avoidable race; the same check (factored into a shared `_read_lock()` helper) now also runs immediately before the save, so a lock lost mid-run (held past the stale threshold — e.g. the machine slept) is reported while it's still actionable. The save still proceeds either way; fail-open is deliberate (F2). `skills/check-items/SKILL.md` Step 10 previously printed `cache updated` only after the whole block, so a body exception or `save_cache()` itself raising (a full or read-only disk) aborted silently — no failure message, no non-zero exit the agent was told to notice, and the terminal output still showed Step 7's stale `Cached: N reused, M fresh` as if it were the final state. Step 10 now catches both, prints `cache NOT updated: <reason>` on **stdout** (mirroring the existing `#320 F2` precedent in the same file — stderr is documented elsewhere in this repo as ignorable) and exits non-zero, with a post-block instruction to stop and report verbatim; the `Output format` block gained an unconditionally-printed `Cache:` line (F3). Three swallowed `OSError`s chained into a worse failure mode: `_release_lock`'s read treated *any* `OSError` as "already gone" (not just `ENOENT`), and a failed `unlink()` in both `_release_lock` and `_acquire_lock`'s stale-takeover branch was silently dropped — an un-removable lock (a foreign owner, a read-only volume, a macOS `uchg` flag) would degrade *every later run* to the full lock timeout and then unlocked operation, forever, indistinguishable from ordinary contention. The read swallow is now narrowed to `errno.ENOENT`, and both unlink failures print a distinct warning naming the path and exception (F4). `save_cache()`'s `NamedTemporaryFile(delete=False)` leaked its temp file if `json.dump`/`flush`/`fsync` raised mid-write — the `finally` closed the handle but nothing unlinked it, and `os.replace()` was never reached; disk-full is exactly the condition likely to produce a whole run of orphans, so the temp file is now unlinked on failure before re-raising (F5). Step 10 also did `cache = update_cache(cache=cache, ...)` inside the `with` block — `locked_cache()` saves its *own* `cache` binding, so the rebind was a no-op today only because `update_cache()` happens to mutate its argument in place and return that same object; the rebind is now dropped (the return value was unused) and `locked_cache()`'s docstring states the in-place-mutation contract explicitly, so a future `update_cache()` that returns a copy fails loudly in review rather than silently persisting stale snapshots forever (F6). **A second review round caught a regression the first round's own #306 fix introduced (F7):** `_lock_age()` moved off `st_mtime` onto the lock payload's own embedded timestamp to close a stale-takeover TOCTOU, but `float()` accepts `"nan"`/`"inf"`, and a backward clock step (an NTP correction, a resumed VM) makes a stored timestamp read as future-dated — all three compare `> _LOCK_STALE_SECONDS` as `False` *forever*, so takeover never fires again and the lock wedges *permanently*: every later run stalls the full timeout and then writes unlocked, which is strictly worse than the pre-#306 baseline (that at least never stalled). `_lock_age()` now applies the same `0 <= age` idiom `partition()` already uses for `classified_ts`, from *either* timestamp source, resolving to a stale sentinel (self-healing on the next acquire) when neither yields a usable past time — with one refinement found while proving the guard by mutation: gating on a bare `0 <= age` turned out to be a real bug of its own, not just a style choice, because `_lock_payload()` formats its timestamp with millisecond precision (`.3f`), which *rounds*, so a freshly-written payload read back microseconds later can compute as a few tenths of a millisecond "ahead" of `time.time()` — measured empirically, a bare `0 <= age` gate rejected roughly **half** of all freshly-written payloads (956,812 of 2,000,000 trials) on that rounding noise alone and fell through to `st_mtime`, reintroducing the exact single-read mis-pairing hazard #306 closed. The gate is now `-0.01 <= age` (`_LOCK_AGE_ROUNDING_TOLERANCE_SECONDS`), two orders of magnitude past the ~0.5ms formatting error and many orders of magnitude short of the minutes-to-hours a real clock step shows, with the returned age clamped to a minimum of `0.0`. Two smaller items from the same round: a contended winner's lock payload was minted once *before* the wait loop began, so it could be born already up to `timeout` seconds "old" the moment its create attempt finally succeeded — silently shrinking its effective stale grace from `_LOCK_STALE_SECONDS` down to `_LOCK_STALE_SECONDS - timeout`; the payload is now minted fresh on every retry attempt (F8). And `_release_lock`'s "this run's lock was taken over" warning asserted the body must have outrun `_LOCK_STALE_SECONDS` to reach it — true usually, but not always, since the documented residual window between `_acquire_lock`'s confirm-read and its unlink can also land a rival's fresh lock there after a body that ran for microseconds; the comment now says so (F9).
- **`/check-items`'s shared cross-project cache no longer loses an entire run's classifications to a concurrent run on a different project (#306):** `load_cache()` → `save_cache(cache)` was an unlocked read-modify-write, and the persistence cache is one global file (`~/.claude/obsidian-brain/check-items-classifications.json`) holding every project's `runs[<project>]` entry. Two runs on different projects overlapping in time could both load the same snapshot, and whichever saved last would write the *whole document* from its own stale copy — silently erasing the other project's run, reproduced with two real processes: `RACE REPRODUCED — ['alpha'] silently lost its entire run`. Severity is low and the failure is fail-safe (every cache entry is a derived verdict, so a lost write just means those items look `new` and get re-classified next run — wasted tokens, never incorrect output), but the fix is worth having to avoid the silent, expensive re-classification. The fix is a `locked_cache()` context manager in `hooks/check_items_cache.py` — not a documented convention in the SKILL, since skills are advisory and code is not — that acquires an O_EXCL lock file derived from `CACHE_PATH`, loads, yields the cache to mutate, and saves + releases once the block exits, so the whole read-modify-write is one operation a caller cannot half-perform. It follows the shape of `claim_hook_run()` (stale-TTL takeover, fail-open on any filesystem error) but differs in two deliberate ways: contention *waits* instead of abandoning (this is mutual exclusion, not dedup — every caller's write must still land), and release re-checks the on-disk payload before unlinking so it never deletes a lock a rival process has since taken over as stale. `skills/check-items/SKILL.md` Step 10 now wraps its per-project cache-update loop in `with locked_cache() as cache:` instead of a bare `load_cache()` / `save_cache(cache)` pair; Step 3's own `load_cache()` is untouched — it's read-only and never written back, so it was never part of the race.
- **`/check-items`'s cache no longer replays a verdict as verified-current when it was never actually re-checked against the repo's latest commit (#305):** `update_cache()` bumps the run-level `project_head_at_classify` unconditionally on every run, even for entries that got no fresh classification this run because nothing about them changed. Since `partition()` only compared that single run-level field, an entry last verified at an older HEAD was silently laundered onto the newer one the next time the run-level field moved — `head_changed` could never fire for it again. Such an entry now carries its own `head_at_classify`, stamped with the head it was actually verified at (and left untouched across further un-reclassified runs, via `setdefault` semantics, so it never inherits a newer head it was equally never checked against); `partition()` reads this per-entry stamp when present and only falls back to the run-level field for entries that did get a fresh record this run. A second, independent laundering path is closed alongside: `skills/check-items/SKILL.md` used to call `git rev-parse HEAD` separately at Step 3 (driving `partition()`) and again at Step 10 (driving `update_cache()`), so a commit landing between the two stamped the newer HEAD onto verdicts derived at the older one. Step 10 now reuses the head Step 3 already captured (carried through `partition.json`'s new `heads` key) instead of re-deriving it. Two review findings are folded in: the `partition.json` handoff itself had no test — deleting `"heads": heads` from Step 3 left the entire suite green while silently disabling *every* cache update from then on, announced only on stderr — and the new per-entry head check runs before the corrupt-timestamp check, so a pinned entry would have skipped the "unusable classified_ts" warning that a clock skew or hand-edited cache should raise. Both are now guarded.
- **`/check-items`'s sibling cascade no longer trusts a stale line number when checking off the OTHER copies of an item you just marked done (#250):** after you confirm an item is done, `/check-items` sweeps its grouped duplicates and `/standup`'s cascade sweeps its own matched duplicates, checking off every sibling line by the (file, line) recorded when it was grouped. If that line had since come to hold a *different* active item — not the one grouped, just something else that happened to land there — the cascade checked it off anyway, because it only verified a checkbox was present, not that the text still matched. `#201` closed this same hole for `/standup deep`'s primary checkoff by re-resolving each target by text; the cascade path is lower-traffic (duplicates are collected and flipped within one run, so drift is rarer) but shares the same class of risk. It's closed differently here, on purpose: `#201`'s fix refuses to act when two or more lines text-match, but a cascade group *is* a set of near-identical lines by design, so that refusal would silently skip the exact siblings the cascade exists to flip. Instead, each sibling is now checked against its OWN recorded text before being flipped — never re-searched for elsewhere in the file — so a line that drifted onto something else is skipped and named in the run's output (not just logged to a place you may never see) while every sibling that still matches still flips, including near-identical duplicates in the same file. A sibling recorded with no text at all is treated as unverifiable and skipped rather than guessed. **A follow-up review round (#320) closed four ways that surfacing itself could go quiet or wrong:** a checkoff whose verified flip failed to reach disk (a full volume, a permissions change mid-run) previously incremented nothing, so the run reported the same wording as "there was nothing to cascade" while silently losing the flip — it's now named in a `WRITE FAILED for N file(s); M verified flip(s) were NOT saved: ...` line, and `/check-items` and `/standup` both now exit non-zero on it instead of reporting quiet success; more generally, once *any* sibling target existed for a run, an outcome where every one of them was refused or lost no longer reads as the empty-input "no member lines to cascade" message — it now says so wasn't nothing, it was refused. A drifted line, one with no text to verify against, and one whose checkbox had already been consumed by something else were previously folded into one undifferentiated "failing text verification" bucket (and the checkbox-consumed case was stderr-only, the likeliest real drift signal and the easiest to miss) — the three are now distinguished by wording and every one of them is named in the return value, not only on stderr. `/check-items`'s own SKILL.md previously discarded every one of these signals down to a bare count on the way into the dashboard (only `/standup`'s had been wired to surface a skip line); its terminal output and its dashboard note (`skipped:` in frontmatter, alongside `cascaded` in the prose summary) now carry the skipped/refused count too. Finally, a stored member `text` that turns out not to be a string (a corrupted cache entry) previously raised `AttributeError` mid-cascade, which — since files are written as each one completes — could leave a run half-applied on top of a user's already-confirmed primary flips; it's now treated as unverifiable and skipped, same as blank text.
- **`/standup deep` no longer strips the location off the three item buckets you actually read (#243):** the deep presentation renders classification groups in a fixed priority order, but that order still named `COMPLETED` and `REDUNDANT` — labels no classifier has emitted since the classification set became `DONE` / `NEEDS-ACTION` / `REVIEW` / `STALE` / `ACTIVE`. So `DONE`, `NEEDS-ACTION` and `REVIEW` matched nothing, and fell through to the trailing catch-all branch meant for labels the tool does not recognise. That branch is not merely unordered: it prints only the item text and its evidence, dropping the project label and the `Found in \`file:line\`` line that the ordered branch prints. The result was that `STALE` and `ACTIVE` — the two buckets needing the least attention — rendered first and complete, while the three decision-relevant ones rendered last and with no way to find the item. The order now covers every valid label, and the set of valid labels is derived from it, so a label cannot be accepted by the validator while being invisible to the renderer. A genuinely unrecognised label still falls to the catch-all rather than being dropped.
- **`/recall`, `/vault-search`, `/vault-ask` and session summaries no longer lose the snapshots a session took before midnight (#70):** a context snapshot is filed under the date it was written, but the session note is filed under the date the session *ended* — so every snapshot taken before midnight in a session that ran past it was simply absent from `/recall`'s history table, from the snapshot summaries `/vault-search` and `/vault-ask` attach to a session hit, and from the material the AI summarizer sees when it writes that session's summary. Nothing said anything was missing; the session just looked shorter than it was. On the live vault, **86 of 641 sessions were undercounting and 192 snapshots were invisible** — worst case a nine-hour session showing **0 of its 9** snapshots, and the two sessions named in the issue going from 0 to 1 and from 1 to 2. Discovery is now date-agnostic: a snapshot is matched to its session by the session id recorded inside it, not by the date in its filename, so it is found whichever day it was written. (`/retro` already worked this way and was unaffected.) The fix is also **faster than the bug it replaces** — reading every snapshot's frontmatter once per listed session would have taken a 10-row `/recall` table from 37 ms to **1.6 s**, so the scan is done once per run and shared: the same table now takes **14 ms**, with the full snapshot section of `/recall` at 106 ms. The scan is shared only on read paths; the SessionEnd hook, which writes a snapshot and then reads the list back to link it, deliberately keeps looking at the disk each time.
- **`/check-items`'s fallback heuristic no longer reads a *pending* issue reference as a completion (#299):** when the AI classifier degrades, the fallback calls an item done if a distinctive reference (`#51`, a commit sha, `v1.2.0`) sits within 120 characters of a completion word (`merged`, `closed`, `resolved`, `shipped`, …). Co-occurrence alone cannot tell *"Fixed in #51"* from *"Waiting on #51 to be closed"* — both shapes match — so items that were explicitly waiting on, blocked by, or merely asking you to verify something were proposed for checkoff. They are now recognised as open: a completion co-occurrence is rejected when the sentence around it is governed by a pending-intent cue (*waiting on*, *blocked until*, *pending*, *depends on*, and the verification verbs *confirm*, *verify*, *validate*, *check*, *track*, *monitor*), or by a time/condition word (*once*, *until*, *if*, *when*, *after*, *whether*) together with a forward-looking verb form (*is resolved*, *to be closed*, *will be merged*, *gets merged*). Past-tense claims are deliberately left alone — *"After the outage we finally fixed #51"* and *"#51 was closed"* still read as done — and a cue only counts when it comes *before* the completion phrase, so *"Fixed in #51 — confirm with the team"* is still a completion with a note attached. Both the completion word and the cues are matched as *prose*, not as substrings: `release` inside the compound *"the post-release-verification item will flip"* is not a completion (a live vault item that the guard would otherwise have newly proposed for checkoff), while a leading hyphen still reads as one (*auto-merged*, *back-merged*) — as does a trailing verb particle, so *"this was merged-in with #51"* and *"#77 was closed-out last week"* are still completions; and `check-pr-base.py`, `scripts/verify.sh` and `` `go.get.it` `` no longer supply a cue out of a filename fragment, while the hyphenated forms people actually write (*re-verify*, *re-check*, *cross-check*, *self-check*) now do, and a slash-joined pair reads as prose rather than as a path, so *"bump the cache if/when #51 is merged"* is recognised as conditional — treating the slash as a path separator matched neither *if* nor *when*, and the item read as done. A wrapped line counts as one sentence — only a blank line ends one — and the dot of a known abbreviation (*e.g.*, *i.e.*, *etc.*, *vs.*) no longer ends one either; both were cutting a cue off from the reference it governs. That abbreviation list is exactly those four — a dot after anything else, including an initial, still ends the sentence, so in *"Awaiting update from A.B. The fix for #51 is merged."* the *awaiting* does not reach across into the completion it has nothing to do with. Replayed over every stored `/check-items` run on this machine (398 distinct item groups), the guard turned 6 of the 22 heuristic "done" verdicts back into open items — including a *"Validate that 9 stranded board items (v3.4.0) …"* task that had never been performed — and turned none of the genuine ones into false opens; replayed over every open item in the vault (1039 distinct lines) it turned 9 of 24 back into open items and, again, created no false opens. When a verdict is rejected, the reason is kept on the item (*which* reference, *which* completion word, *which* cue) and printed to the run log, rather than the item quietly reverting to "active" with no explanation — including the two cases with no cue to name: where the reference and the completion word sit in different sentences and the guard declines to judge them at all, and where the completion word turns out to be only part of a hyphenated compound, so no completion claim survives *near that reference* to judge — *"note `(merged-on-PR-close)` in tracking row. PR #859 was avoidable."* is not a claim that #859 was merged. That second case was created by this guard's own compound rule: swept over every open item in the vault together with every stored run (3671 distinct texts), 2 items had reverted to "active" with nothing said about why, and both now say why. Where an item has several lines and more than one is disputed, the *informative* objection is the one reported — a named cue outranks "these two sit in different sentences", which in turn outranks "that completion word was only part of a compound" — so the explanation no longer depends on which line happened to be listed first; and every guard log line quotes the item itself rather than only an internal group id, with the two kinds of line (a refused verdict, versus a done item carrying an objection to one of its other lines) labelled distinctly. Finally, when one line of a grouped item reads as done and another is *disputed* — a named cue, or a reference and completion word the guard refused to read together because they sit in different sentences — that objection is now carried on the group and logged instead of being discarded — and it names that line by file and line number: checkoff cascades to *every* line of a group, so the disputed line is precisely the one you need told about, and *which* line it is is the part you cannot work out from the group's own text. The compound case is deliberately not raised there: it means the line had no completion wording near a reference at all, which is the same state as a line that never mentioned completion — and those raise nothing — so flagging one and not the other would only dilute the warning on the lines that really are disputed.
...
- **`/check-items` no longer silently answers about the wrong project on a mistyped or unrecognised argument (#318):** an unknown token — a typo, or a notes-only project with no directory under a workspace root — fell off the end of `parse_scope`'s loop with no record, so the run silently fell back to the current project's scope and answered about that instead. Unknown tokens are now recorded on `scope.unknown_tokens`, checked against the union of workspace-root projects and every vault-index project (so a notes-only vault project is recognised, degrading to empty on any lookup failure rather than raising), and Step 1 now exits 2 naming the offending token(s), a substring-plus-`difflib` "Did you mean" suggestion when a known project name is close (`obsidian-brian` -> `obsidian-brain`), and the valid-forms list — instead of proceeding silently against the wrong scope.
- **`/check-items` no longer silently skips every git-derived evidence source for a project with no local repo (#318):** `deep_analysis_pipeline` used to `continue` past any such project, capping every one of its items at tier LOW with classification DONE structurally unreachable — while the run still printed a normal-looking triage report with nothing to say why. The loop body no longer bails early, and a new `evidence_gaps` block in the pipeline's output JSON (`projects_scanned`, `projects_with_evidence`, `projects_without_repo`, `all_projects_gapped`) plus a stderr warning names the gap. The dashboard now surfaces this too: a `## Evidence Gaps` section — rendered only when there's an actual gap to report, never unconditionally — and an `evidence_gaps` frontmatter field, so the artefact the user keeps, not just a stderr line, says a repo-less project's run was a precondition failure, not a clean triage.
- **`/check-items`'s dashboard no longer crashes when a semantic-merge count reaches it instead of a list (#318):** the report writer iterated `merges` directly, so a caller passing an `int` raised `TypeError: 'int' object is not iterable` — and there was no correct value to pass in the first place, since `merge_groups_semantically` only ever returned surviving groups carrying `absorbed_reasoning`, never the `canonical_group_id`/`absorbed_group_ids`/`reasoning` records the `## Merged Groups` section reads. A new `merge_records_from_groups()` adapts one shape to the other; the report writer now normalises `merges` (`None` reads as no merges this run, anything else non-list produces an explicit `**Merged Groups unavailable**` warning naming the bad input, never silently coerced to "nothing to report").
- **`/check-items`'s dashboard report no longer misattributes which items were actually applied (#318):** the report rendered `- [x]` for every DONE classification and `- [ ]` for everything else, regardless of what the user actually confirmed. Opting a REVIEW or NEEDS-ACTION item into checkoff flipped it correctly in the vault, but the report still showed it unchecked and folded its count into DONE's heading — the vault was correct, only the report, the artefact the user keeps, lied. Every checkbox now renders from whether the item was actually applied — Step 8's primary-flip loop stamps this per record and persists it before the dashboard reads it — not from its classification; DONE's heading counts only applied DONE items, and a new summary line names flips applied outside DONE so they don't vanish from the arithmetic.
