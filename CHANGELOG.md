...
- `vault-doctor` `sessions_by_id` in both `snapshot_migration.py` and `snapshot_integrity.py` no longer silently picks a last-write-wins parent when two session notes share a `session_id`. Colliding sids are tracked in a `_sid_collisions` set at build time, and all four consumers (migration §3 / §4 and integrity §1 / §4) now emit unresolved `"ambiguous — multiple session notes share session_id=<sid>"` Issues instead of proposing a confidently-wrong backlink or snapshots-list fix. Surfaced during PR #80 review as a pre-existing weakness made consequential by the #68 fix. Closes #81.
