...
### Fixed
- **tfidf recompute on bulk deletions (#235):** the corpus IDF is now recomputed after a sync that *deletes* more than half the corpus, not only one that inserts. Previously a large pruning left surviving notes with stale pre-deletion IDF, skewing similarity/clustering until the next bulk insert.
