...
### Fixed
- **A refspec push, or a global `git` option, walked straight past the gates that exist to stop them (#327):** `prevent-direct-push` built a refspec-aware list of protected targets — `:main`, `:develop` — under a comment claiming refspec coverage, and then guarded the deny with a **strictly narrower** `if` (`current_branch in [...] or "origin main" in command or "origin develop" in command`) that no refspec form can satisfy, so every one of them fell through to allow. Measured hermetically from a feature branch against the unfixed code: `HEAD:main`, `HEAD:develop`, `mybranch:main`, `+main:main`, `--force HEAD:main` and `--force-with-lease HEAD:main` were all permitted — a force-push over `main` was, in practice, an allowed operation. There is one condition now, and it is the refspec-aware one; `HEAD:refs/heads/main`, the same ref written long, is covered with it. **The same defect is wider than the title.** All three git gates matched their verb as a literal substring, and git accepts global options *between* the executable and the subcommand — so `git -C . <verb>`, `git -c user.name=x <verb>` and `git --no-pager <verb>` contain no such substring, and the hook exited 0 before ANY other logic ran: for the push gate, the branch-name gate (`git checkout -b`) and the preflight gate (`git commit`) alike. `git -C . <push> origin main` pushes THIS repository and was allowed outright. The verb is now matched with a pattern that tolerates git's global options and a path to the executable (`/usr/bin/git …`), and the SAME pattern is handed to the #326 scope helper, so the gate and its scoping can no longer disagree about where the verb is — a disagreement the helper's `if not verb_positions: return True` swallowed as "gate it": fail closed, correct, and silent. **Two changes turn a deny into an allow, deliberately.** `"origin main" in command` is a prefix test, so pushing the ordinary branch `maintenance` DENIED; ref comparisons are now anchored to a whole ref on both sides (`foo:mainline` was already allowed and still is). And a verb now only counts where a shell would actually RUN it: a command that merely QUOTES or echoes a protected push — `echo <push> origin main`, `grep '<push> origin main' .`, the same string inside `"…"` — denied before and allows now. That false deny repeatedly blocked this project's own tooling. It is not a hole in the other direction: forms that really do run the verb stay gated (`sudo <push>`, `env A=b <push>`, `time <push>`, `xargs -n1 <push>`, a push inside `$( )`), and a prefix that does not parse KEEPS the occurrence, because a dropped verb is an allow. **Two fail-opens in the same files, surfaced by the deep review of #325.** The `git`/`gh` subprocess handlers caught `(CalledProcessError, FileNotFoundError)` — but `PermissionError` is equally an `OSError` and was not named, so a `git` on `PATH` that exists and is not executable made the hook exit 1, which is a NON-blocking error under the PreToolUse contract, and the push proceeded. Widened to `OSError` in `prevent-direct-push` and at all three sites in `enforce-pr-base-branch`. And the allow-side output sites — `require-preflight`'s audit line and `update-changelog-before-pr`'s `add_context` — were still on a bare `print()` while the deny side flushes inside a try and falls back to exit 2; both now go through one `_emit()`, so a dead pipe on a path where nothing is wrong no longer yields `rc=120` and an `Exception ignored on flushing sys.stdout` in the transcript.
