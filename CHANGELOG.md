...
### Fixed
- **Every `.claude/hooks` gate now decides by shell grammar instead of by substring, in both directions (#327):** five PreToolUse gates guard this repository, and each of them answered "is this command mine?" and "may I let it through?" by looking for characters anywhere in the command text. Sixteen defects followed from that one habit. All were measured hermetically — a throwaway repo on a feature branch, because probing from `develop` makes `current_branch in ["main", "develop"]` deny every shape and no two rows can diverge — and every row below is pinned by a test whose reference verdict is `develop`'s.

  **1. The verb was matched as a literal, so any command that spelled it differently never reached the gate.** `prevent-direct-push` tested `"git push" not in command`, and git accepts global options between the executable and the subcommand: `git -C . push origin main` pushes THIS repository to main and was allowed outright, exiting 0 before any other logic ran. The same shape gated `git checkout -b` and `git commit`, so a non-conforming branch name and an untokened commit were reachable the same way, and `gh --repo o/r pr create` (verified against the real binary) evaded both PR gates. Also never matched: a path to the executable (`/usr/bin/git push …`), a line continuation (`git \`+newline+`push` — `\s+` does not span the backslash, and the guard ran before the helper that has collapsed continuations since #326), and `git checkout -B`, `git switch -c/-C` and their long forms `--create`/`--force-create` — six spellings of "create a branch", of which the gate knew one. The verb is now one pattern, tolerant of global options and of the executable's path, shared by the gate and by its scope check so the two cannot disagree about where the verb is.

  **2. The protected-branch test never saw a refspec.** A refspec-aware list of targets was built — and then guarded by a strictly narrower `if` that no refspec form could satisfy, so every one fell through to allow: `HEAD:main`, `HEAD:develop`, `mybranch:main`, `+main:main`, `--force HEAD:main`, `--force-with-lease HEAD:main`, `HEAD:refs/heads/main`, `:main`. Force-pushing over `main` was, in practice, permitted. The rule also keyed off the remote NAME, so `git push upstream main` and `git push git@github.com:o/r.git main` were allowed. A protected ref is now matched as a whole WORD among that push's own arguments, on any remote, quoted or not, with its peel and reflog suffixes (`main^{}`, `main~1`).

  **3. A verb that a shell would not run still counted.** `echo git push origin main`, a push inside quotes, and `grep '<push>' .` all denied — repeatedly blocking this project's own tooling. A verb now counts only at a command position the shell would execute. Getting that right is most of this change: `if <verb>; then`, `while`, `until`, `exec`, `\<verb>`, `sudo`, `env A=b`, `command`, `nohup`, `time`, `eval`, and wrappers with arguments of their own (`timeout 60 <verb>`, `nice -n 10 <verb>`, `xargs -n 1 <verb>`) all run the verb and stay gated; a verb inside `$( )` or backticks runs and stays gated — **including inside double quotes, which is the third time this clause has had to be made true**. `_shell_scan` jumped over a `"..."` span in one step, so nothing written inside it ever reached the stack: `echo "$(<verb>)"` and `` echo "`<verb>`" `` read as ordinary quoted text, the occurrence was dropped as quoted, and the hook exited 0 with no decision — an ALLOW, on `develop`-denied shapes, and command substitution IS performed inside double quotes so they really run. The double quote is a toggle now, `'` and a bare `(` are literal inside one, and the substitution test runs BEFORE the quote test so that `echo "$(` is `subst` and keeps its verb rather than `quoted`, which would drop it. A verb that is merely quoted, with no substitution around it, still allows; a command that does not parse keeps every occurrence, because a dropped verb is an allow. **The same walk had three holes where NOTHING stood in front of the verb**, each of which ran the verb, denied on `develop`, and opened all five gates. A **multi-line** command: `echo starting` + newline + `<verb>` allowed, because the walk stripped trailing whitespace — including the newline it was about to test as a separator — and then read a live command as `starting`'s neighbour. A two-line Bash command is the ordinary case, so this was the widest of the three; it survived because a newline with nothing before it denies correctly, and no test had put text on the first line. A **backtick**: `` `<verb>` `` allowed while `$(<verb>)` denied, because `(` was in the separator set and the backtick was not — so the claim above about backticks is true of the shipped code, and was not true of the first cut of this change. A **leading redirection**: `>/dev/null <verb>`, `2>/dev/null <verb>`, `<in <verb>` and `>>out <verb>` all allowed, because a redirection is not a command name and the walk read it as an unrecognised bare word. **File-descriptor duplication was a second, separate hole in the same place**: `2>&1 <verb>`, `1>&2 <verb>`, `3>&1 <verb>`, `N>&M-` and `N<&M` all ran the verb and all allowed, because `&` was read as the command separator a bare `&` is — leaving the file descriptor `1` behind as an outstanding bare word, so the verb read as that word's argument. The `&` in `>&`, `<&` and `&>` is part of the operator and no longer separates anything; `&&` and a lone `&` still do, and only adjacency makes an operator, exactly as in bash, so `echo a & >out <verb>` is still a background `&` followed by a separate redirection. **The same misreading cut a push short of its own arguments**: `<push> 2>&1 origin main` ended the command's span at the `&`, so the protected-ref test — which reads only that span — never saw `main`. It allowed here and denied on `develop`, which is the shape of a regression rather than of a fix; a redirection with its target attached now claims no word of its own, while a bare `>` claims exactly the one word to its right, so `echo a > out <verb>` is still `echo`'s data. A `#` comment gets no special case, deliberately: the verdict comes from the real command word to its left (`echo x # <verb>` allows, and a `#` on the line BEFORE the verb is settled by the newline), because a rule that allowed on merely SEEING a `#` would be quote-blind — `env "A=x # y" <verb>` really does run the verb.

  **3b. Judged occurrences, PR numbers, and the cost of the scan itself.** Three more ways a gate stood down, all measured. **Only the FIRST occurrence was judged** by the branch-name gate and the PR-base gate; the push gate had been fixed to loop and these two were left behind, so one acceptable command laundered every later one on the same line. `git checkout -b feature/ok && git checkout -b badname`, `git checkout -b release/v1.0.0 && git checkout -b release/1.0`, `gh pr create --base develop && gh pr create --title x` and `gh pr create --base develop && gh pr create --base main --title x` all allowed, while each offending half denies alone — and the last of them opens a feature→main PR, the single thing that gate exists to prevent. Every occurrence is judged now, in both gates and on the merge path too. **The PR number was the first digit run in the span**, so a digit inside a flag's value decided which PR got its base checked while a different one was merged: against a stubbed `gh` where #331 targets develop and #296 targets main, `gh pr merge -t "Merge PR 331" 296 --squash` and `gh pr merge --body "closes 331" 296 --squash` both verified #331 and merged #296. The number is now read from the BARE words of that merge's own arguments — quoted words and the value of a value-taking flag (`-b/--body`, `-t/--subject`, `-R/--repo`, `--body-file`, `--match-head-commit`) are skipped — and a span naming two different PRs is refused rather than guessed at. In the allow direction, `gh pr merge -t "Merge PR 296" 331 --squash` now merges: the blocked number is a word of a subject, not the PR. **And the scan itself was quadratic.** `_shell_scan(cmd[:start])` restarts at offset 0 and ran once per match: 0.33s at 14 KB, 1.72s at 32 KB, 5.27s at 56 KB, with 1 MB inside the stdin cap these hooks accept — and the separator-free shape was far worse, 7.8s at 32 KB and 488s at 256 KB, because every step of the command-position walk copied and rescanned all the text to its left. A hook the harness kills has written no decision, and no decision is an ALLOW, so slowness was itself a bypass. Prefix states are computed once in a single pass (`_shell_states`, pinned equal to the old scanner at every offset of a corpus by its own test), and the walk moves by index without looking past the nearest boundary. Same shapes now: 0.17s at 32 KB and 1.1s at 256 KB. A **32 KB command-length cap** backs that up in all five gates, checked before the verb is matched so it also covers the cost of matching; over it, the gate blocks. That is fail-closed on a long command a gate would otherwise have ignored — a heredoc writing a large file, say — and it is one constant if that trade is wrong for you.

  **4. Every allow-side escape hatch was still a raw substring, so each was an off switch for the whole command line.** `<push> origin main  # see refs/tags/v1` allowed. `echo '--tags' && <push> origin main` allowed. `<push> --delete origin release/x && <push> origin main` excused the second push with the first one's flag. `<commit> -m "document SKIP_PREFLIGHT=1 escape hatch"` skipped preflight — by accident, just by writing about it. `echo --base develop && gh pr create --title x` satisfied the base-branch check for a PR that GitHub then defaults to main, which is the thing that gate exists to stop. And `gh pr merge --squash && echo 123` verified PR #123 while merging whichever PR the branch points at. Each hatch is now read from the arguments of the command that carries it, and `SKIP_PREFLIGHT=1` additionally has to be an assignment the shell would really make — judged by the same command-position walk as the verb, so `env SKIP_PREFLIGHT=1 <commit>` and `sudo SKIP_PREFLIGHT=1 <commit>` keep working while `echo SKIP_PREFLIGHT=1 && <commit>` and `SKIP_PREFLIGHT=12` do not.

  **5. Three fail-opens where an exception escaped.** The `git`/`gh` subprocess handlers caught `(CalledProcessError, FileNotFoundError)`, but a `git` on `PATH` that exists and is not executable raises `PermissionError` — equally an `OSError`, and not named — so the hook exited 1, which is a NON-blocking error, and the push proceeded. A preflight token that parses but is not an object (`[]`, `5`, `"x"`) raised `AttributeError` on `.get()`, and a non-numeric `expires` raised `TypeError`; neither was caught, both exit 1, and the unverified commit went through. Both widened, and the token's shape is now validated before use.

  **6. One output path for the decision channel.** `require-preflight`'s audit line and `update-changelog-before-pr`'s `add_context` were still bare `print()` while the deny side flushed, verified and fell back to exit 2. Both now go through one `_emit()`, so a dead pipe on a path where nothing is wrong no longer yields `rc=120` and an `Exception ignored on flushing sys.stdout`.

  **6b. Two quadratic patterns, and only one of them was the scan.** The optional path prefix `(?:[^\\s;&|()<>]*/)?` in all five gates made the regex engine scan forward from every offset looking for a `/` that is not there — quadratic **with zero matches**, so it fires on command text no gate would ever have judged. Measured on `"a" * n`, no match anywhere in it: 0.029s at 4 KB, 0.451s at 16 KB, 7.205s at 64 KB, 115.7s at 256 KB, against a 1 MB stdin cap. End-to-end, a plain `echo <64 KB of base64>` cost 7.29s inside the hook and then allowed. The trigger is mundane — a base64 blob, a data URI, a minified bundle, a `--data` payload. This is a DIFFERENT defect from the quadratic scan above: it fires before `_shell_scan` or the command-position walk is reached at all, so the single-pass scan leaves it completely in place, and each has its own test. The path component may now only START at a word start (a fixed-width negative lookbehind), which makes the pattern linear — 0.0003s / 0.0005s / 0.0017s / 0.0069s at the same sizes — and removes no match: the leftmost match always began at a word start already, verified as identical start offsets on 20,022 probes including `/usr/bin/git`, `./git`, `mygit`, `x/mygit`, `gitd`, `git/x`, `a/b/c/git`, `$HOME/bin/git`, `'git'` and `github`.

  **6c. The escape hatches were substrings one level down, and `--base` took the first match.** Scoping each hatch to the command that carries it was half of item 4. Within that span the tests were still `"--tags" in span`, so hatch text sitting in the SAME push's argument list stood the gate down — and all four of these are real git that really does push main, since `--tags` pushes tags IN ADDITION to the refspec rather than instead of it: `<push> origin refs/tags/v1 main`, `<push> --tags origin main`, `<push> --delete origin release/x main`, `<push> --force origin main refs/tags/v1`. The span does not end at a redirect or a process substitution either, neither of which is an argument to `git push`, so a hatch could also be read out of a filename or a subshell: `<push> origin main 2> refs/tags/err`, `<push> origin main <(echo --tags)`, `<push> origin main --push-option="see --tags"`. A hatch now has to match a whole argument WORD, the word list stops at the first redirection, and — the change that actually closes the class — a hatch is consulted only for a push that names NO protected ref, so a push carrying both a hatch word and `main` is denied on the `main`. What the hatch is for is unchanged and still works: a version-tag push, a `refs/tags/*` push and a `release/`/`hotfix/` branch deletion, from `main` as well as from a feature branch. Separately, `--base` was read with `re.search`, which takes the FIRST match, while `gh` honours the LAST: `gh pr create --base develop --base main` created a feature→main PR and allowed, while the reversed spelling denied — an asymmetry that proves it was first-match rather than a rule. Two different bases in one create are now refused rather than guessed at (the same base twice is fine), and `--base` is read by word, which also fixes a false deny nobody had noticed: `gh pr create --base develop --title "not --base main"` used to read `main"` out of the title.

  **6d. The scanner did not know what a comment or a here-doc body IS.** One apostrophe in a comment opened a quote span and a second one closed it, so the command as a whole parsed, the verb between them read as quoted, and the occurrence was dropped — exit 0, no decision, which is an ALLOW. `# don't` + newline + the verb + newline + `# it's fine` stood all five gates down, verified with a `git`/`gh` shim on PATH that records what it was asked to run. The double-quote spelling denied even before the fix; one apostrophe is what it took. The parity rule this relied on (`whole_parses`) closes the ODD case and nothing else, and its docstring said otherwise — that is corrected too. Comments and here-doc bodies are modelled now, and they are NOT the same verdict: a comment is provably inert, so its verb is dropped outright, while a here-doc body is data to this shell but `bash <<EOF` runs it, so a verb there is kept and gated. On the allow side the same gap let `SKIP_PREFLIGHT=1` written into a here-doc body disable the preflight gate with no token present; the hatch now requires a position the scanner calls `exec`, and a body is not one. **Two more spellings of an assignment prefix** were unrecognised — `A+=1 <verb>` and `A[0]=1 <verb>` both run the verb and both allowed, because `partition("=")[0].isidentifier()` knows only `A=1`. **Three more exec wrappers** join `sudo`/`nice`/`timeout`: `caffeinate`, `arch` and `script`, each verified by running it against `touch` and watching the file appear, and `script` takes a positional file of its own so it is in the argument-taking set as well.

  **6e. A third quadratic site, and a span cut by an OPENING delimiter.** `_argument_span` asked `_shell_scan(cmd[start:i])` once per candidate separator and that scanner restarts at offset 0: 1.09s at 5 KB, 4.23s at 10 KB, 16.97s at 20 KB — all of it under the 32 KB length cap, so the cap never fired, and a hook the harness kills writes no decision. This is the THIRD such site and it is distinct from the other two; each has its own test. The same helper also could not tell a delimiter that OPENS from one that closes, so `<push> origin ${FORCE} main` ended its span at the brace and `` <push> origin `:` main `` ended it at the first backtick — in both, the protected ref sat outside the span the gate reads, and both allowed. The span is now decided against the single-pass state vector: a candidate ends the command only when the shell is at the same level there as it is at the start, `${...}` is tracked so its `}` is not the end of a command, and a backtick ends the span only when it CLOSES the substitution the span is inside. Measured after: 0.061s at 20 KB.

  **6f. A stale stack snapshot in the state vector.** The `("exec", tuple(stack))` snapshot was cached and rebuilt only when the stack's LENGTH changed, so a pop followed by a push returned the previous tuple — `` (`)(` `` is the shortest string that shows it, at five characters. No verdict was found that changed, but it violated the equality the equivalence test asserts, and that test's corpus was exhaustive only to three: green against a scanner that was already wrong. The key is the stack itself now, and the corpus runs to five.

  **7. Five fail-opens outside the parser.** **No subprocess call in any of these hooks had a timeout,** including the ones that go to the network; `subprocess.TimeoutExpired` is a `SubprocessError` but neither a `CalledProcessError` nor an `OSError`, so every handler naming those two let a timeout escape as rc 1 — non-blocking. Timeouts are 10s for `git`, 30s for `gh`, every handler is widened, and a timeout on the merge path denies. The bound itself is pinned by tests that stub a `git`/`gh` which never answers and require a block with the reason that says so — `timeout=` alone was not enough, because `timeout=None` is no timeout at all and left the original unbounded hang in place with the whole suite green. Only the three hooks that shell out carry a timeout constant: `require-preflight` and `validate-branch-name` make no subprocess call and do not import `subprocess`, which is asserted rather than assumed. **"Cannot determine the branch" fell back to `""`,** with a comment claiming that was "safe in the deny direction". It was not: `""` is not `main`, not `develop`, and starts with neither `release/` nor `hotfix/`, so a push whose only offence is the branch it is ON went through — measured with a non-executable `git` on PATH, `git push` and `git push -f` from `main` both denied with a working git and both ALLOWED with a broken one. The same `""` made `"".startswith("feature/")` False in the PR-base gate, so no base check ran at all. Both now block; a detached HEAD, which is rc 0 with empty output, keeps its old meaning. **The preflight token read caught `(ValueError, OSError)`,** so a token file of 200,000 `[` characters raised `RecursionError` out of `json.load` — uncaught, rc 1, unverified commit allowed. The path is a predictable, world-writable `/tmp` name, so writing that file needs no privilege. The handler is `except Exception` now, and a token over 64 KB is refused before it is parsed. **`json` accepts `NaN` and `Infinity`,** and both passed the "is the expiry a number" test; `current_time > NaN` is False, so the token never expired. Non-finite expiries are rejected, and the comparison is written as `not (current_time <= expires)` so anything incomparable lands on the expired branch. **`_get_token_path()` calls `os.getcwd()` at import time,** which raises when the working directory has been deleted — before any code that could decide anything exists, so again rc 1 and again non-blocking. It is guarded, and a token path that cannot be named blocks.

  **8. One "cannot determine" deliberately stays on the allow side, and is now named as such.** When neither `origin/develop` nor `origin/main` resolves locally, the changelog gate cannot tell whether the `[Unreleased]` entries are new on this branch or stale, and it allows. Unlike every other unavailable input in this change, that one is not reachable from the command being judged — it is a property of the checkout — and the requirement the gate exists to enforce has already passed, since there ARE entries. Failing closed there would block `gh pr create` in every repo whose remote-tracking refs have not been fetched, on a changelog that is, as far as anything can tell, correct. The reason string now says the staleness check was skipped, and that text is surfaced on the allow side rather than left silent.

  **9. A differential fuzzer is part of the suite now.** The suite was 3035 tests green while 16 of 55 executing shell constructs — 29% — walked past the push gate, because no fixture in it contained a newline or a backtick. The new test builds constructs by combination, asks BASH whether the verb really executed (with a filesystem side-effect as the marker: stdout is captured inside `$( )` and backticks, and would report "did not run" for exactly the constructs that hide a live verb), and requires a deny for every construct bash runs. It is deterministic — a fixed product of prefixes, suffixes and wrappers, not random generation — and it is what caught the file-descriptor hole above.

  **Behaviour changes in the ALLOW direction, all deliberate.** These commands DENIED before and are permitted now: a verb that is another command's argument or is quoted (`echo <verb>`, `echo '<verb>'`, `grep '<verb>' .`) on all five gates; `git push origin maintenance` and `git push origin develop-x`, ordinary branches that merely start with a protected name; `mygit push …` and `legit push …`, which are not git; and `git pushd …`, which is not a push. Quoting no longer hides a ref from the protected-branch test in either direction: `git push origin 'main'` and `--force origin "HEAD:main"` deny, while `origin 'maintenance'` and `origin "foo:mainline"` allow. Two hatches now apply more NARROWLY than before, which can deny what used to pass: a tag/`--delete`/version-tag allowance excuses only the push whose own arguments carry it, and `SKIP_PREFLIGHT=1` only counts as an assignment at a command position. Known and deliberate limits, documented in the code: a quoted executable (`"git" push …`) is not matched, and neither is a verb inside a string a shell later executes (`eval "<verb>"`, `bash -c "<verb>"`).
