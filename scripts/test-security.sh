...
run_test "C1: _ensure_secure_dir creates 0o700 directory" \
    python3 -c "
import sys, os, stat; sys.path.insert(0, sys.argv[1])
...
run_test "H1: write_vault_note blocks ../ traversal" \
    python3 -c "
import sys, os, tempfile; sys.path.insert(0, sys.argv[1])
...
with open(os.path.join(sys.argv[1], 'obsidian_session_log.py')) as f:
    src = f.read()
...
run_test "M6: session_hint stdin capped" \
    python3 -c "
import os, sys; sys.path.insert(0, sys.argv[1])
with open(os.path.join(sys.argv[1], 'obsidian_session_hint.py')) as f:
    src = f.read()
...
run_test "M6: context_snapshot stdin capped" \
    python3 -c "
import os, sys; sys.path.insert(0, sys.argv[1])
with open(os.path.join(sys.argv[1], 'obsidian_context_snapshot.py')) as f:
    src = f.read()
...
run_test "M7: vault_index escapes LIKE wildcards" \
    python3 -c "
import os, sys; sys.path.insert(0, sys.argv[1])
with open(os.path.join(sys.argv[1], 'vault_index.py')) as f:
    src = f.read()
