...
2. Deliver full parity across every skill in the §9 table (19 at baseline `184715d`) and automatic capture/recovery. Intermediate milestones are not a parity release.
...
| At `184715d` there are 96 `test_*.py` files; #367 adds a 97th for the interim lifecycle guard. `setup.cfg` excludes `hooks/obsidian_utils.py` and `hooks/deep_cli.py` from the coverage threshold. `CLAUDE.md` was corrected in #361. | New runtime modules must not be added to those exclusions. |
...
#362/#363 shipped an interim guard in `obsidian_utils.py`: `_CODEX_HOST_MARKERS` checks `CODEX_THREAD_ID`, `CODEX_SANDBOX`, and `CODEX_SANDBOX_NETWORK_DISABLED`; `_resolve_session_id` returns `unknown` and `check_hook_status` short-circuits when a marker is set, while `_UNCACHEABLE_SIDS` keeps `unknown` out of the session cache. A `codex exec` shell on 0.155.1 had all three markers and an inherited Claude session ID. This guard prevents that Claude ID from being stamped by Codex, but it also makes Claude-inside-Codex resolve `unknown`. #367 adds an interim skip to the four Claude lifecycle handlers, using the hook payload first: a rollout path under `$CODEX_HOME/sessions/**/rollout-*.jsonl` identifies Codex; a Claude path under `~/.claude/projects/**` must not be overridden by inherited markers. A marker is only a fallback when no transcript path is present. SessionEnd writes structured `SKIPPED_CODEX_HOST` telemetry; SessionStart, PreCompact, and Stop report the outcome on stderr only. Explicit host context replaces both heuristics when the adapters ship. Until then, diagnostics must distinguish an interim skip from an ordinary missing session.
...
Keep `.claude-plugin` packaging and one marketplace source. Codex 0.155.1 currently accepts this repository through legacy compatibility with `.claude-plugin/marketplace.json`, snapshots it into `~/.codex/plugins/cache/`, and auto-discovers `hooks/hooks.json`. In the installed cache, all four Claude handlers are trusted and run under Codex with Claude semantics; they are registered **once**, not twice. The Claude `SessionEnd` handler then rejects Codex rollout paths as `SKIPPED_TRANSCRIPT_OUTSIDE_PROJECTS`, while SessionStart and Stop also run the wrong host policy. #367 adds an interim payload-aware `SKIPPED_CODEX_HOST` exit to those handlers; the installed cache must be refreshed before users receive it. The OpenAI package must declare its Codex hooks explicitly so the Claude `hooks/hooks.json` is never selected; an installed-package test asserts the selected hook file and executes the wrappers. The project-level `.codex/hooks.json` added in #361 registers Git policy hooks when Codex works in this repository, but it has no trust entry on the tested machine and is not the future package lifecycle manifest. Decide whether that project file remains alongside, or moves into, the explicit plugin hook selection; setup and vault-doctor report its trust state either way.

OpenAI documentation supports root `plugin.json` with `extensions.com.openai` and `.codex-plugin/plugin.json` as a compatibility fallback. Do not choose between them from file layout alone: §11 step 1 tests manifest precedence alongside `.claude-plugin/` and explicit hook selection in both installed clients. The same step checks whether the existing `.claude-plugin/marketplace.json` works in desktop's bundled `0.155.0-alpha.9.2` as it does in CLI `0.155.1`; keep it as the single source if so, or choose `.agents/plugins/marketplace.json` and migrate both clients together. Share skill resources, and do not treat Claude alias environment variables as host evidence. [Plugin packaging](https://developers.openai.com/plugins/build/plugins)
...
Every §12 check, including behavior conformance and installed-package selection, runs in CI on every PR and becomes a required status check on `develop` before parity is declared. Release-time checks are the recorded desktop live run and, only if weekly automated fresh-fixture capture is unavailable, the explicit fixture/recontract check described in §12.
...
| Both-host hook contracts | A parameterized pytest runs each host wrapper as a subprocess with sanitized golden fixtures under `tests/fixtures/hosts/<host>/<event>.json`. It asserts exit code, output schema, and wall time, reading the configured Codex `SessionEnd` timeout from the **repository's** Codex manifest in unit CI and from the installed copy in the package-install job, with a safety margin beneath it. Include a blocking Codex `Stop` retro-gate fixture with `stop_hook_active` loop protection. Pin client version and capture date with each fixture. The matrix requires fixtures for every supported event; a live desktop run still verifies installation and trust. |
| Both-host behavior conformance | A `conftest.py` `host` fixture runs tests through capture, the writer, `brain_cli.py`, and skill snippets for `host ∈ {claude, codex}` by default. A collection-time check fails any test on those paths that lacks host parametrization or an explicit `@host_only(<host>, reason=…)` marker; the matrix must record a matching `unsupported:<reason>` decision for every opt-out. Store expected notes under `tests/fixtures/hosts/golden/`, so both outputs are checked against a fixed oracle as well as each other. Feed the same logical session through both transcript adapters. For comparison only, normalize provenance frontmatter (`agent_provider`, `agent_session_id`, native `session_id`/`source_session`, and host-specific backlink targets), replace the host-qualified filename digest with a placeholder, and map native tool names such as Claude `Bash` and Codex `shell` to the same semantic category; separately assert that raw records retain native names. Compare all remaining content, including filters, frontmatter, summaries, and skill outcomes. In CI, install the pinned Codex CLI from the checkout, list the installed plugin and its declared skills/hooks, and assert that the Codex manifest selects its wrappers rather than Claude's lifecycle hooks; verify the no-auth installation path in §11 step 1. |
