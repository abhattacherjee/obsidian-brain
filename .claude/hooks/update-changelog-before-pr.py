...
def main():
    ...
    # separator -- so `gh \<NL> pr create` is `gh pr create`. Normalise
    # ONCE here, before the entry test below and the --base regex it
    # feeds: the entry test exiting 0 with empty stdout is an ALLOW, and
    # it read un-collapsed text before `_targets_this_project` ever got a
    ...
    if re.search(_PR_CREATE_VERB, command) is None:
        sys.exit(0)
